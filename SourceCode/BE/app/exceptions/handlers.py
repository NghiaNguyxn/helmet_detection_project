...
from SourceCode.BE.app.exceptions import auth, base, helmet
from SourceCode.BE.app.exceptions import user
...
def register_exception_handlers(app: FastAPI):
    ...
    @app.exception_handler(auth.NotAuthenticatedError)
    async def auth_exception_handler(request: Request, exc: auth.NotAuthenticatedError):
        return create_error_response(
            code=status.HTTP_401_UNAUTHORIZED,
            message=exc.message,
    ...
    @app.exception_handler(user.UserCannotUpdateStatus)
    async def user_cannot_update_status_exception_handler(request: Request, exc: user.UserCannotUpdateStatus):
        return create_error_response(
            code=status.HTTP_403_FORBIDDEN,
            message=exc.message,
            error_code="USER_CANNOT_UPDATE_STATUS"
        )

    @app.exception_handler(user.UserCannotDelete)
    async def user_cannot_delete_exception_handler(request: Request, exc: user.UserCannotDelete):
        return create_error_response(
            code=status.HTTP_403_FORBIDDEN,
            message=exc.message,
            error_code="USER_CANNOT_DELETE"
        )
